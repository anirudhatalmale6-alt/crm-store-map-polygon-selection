...
-- Check first with:
--   SHOW COLUMNS FROM `client_address` LIKE 'lat%';
--
...
CREATE INDEX `client_address_latlng_idx` ON `client_address` (`latitude`, `longitude`);
...
-- re-geocoding.
-- -- Rollback for `client_address`
-- DROP INDEX `client_address_latlng_idx` ON `client_address`;
-- ALTER TABLE `client_address`
