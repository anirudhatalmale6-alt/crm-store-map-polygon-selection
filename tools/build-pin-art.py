...
def main(d='.'):
    ...
    for key, stem in SRC:
        ...
        print(f'{key:10s} {sm.size} {colour} head={head} {len(buf.getvalue()):6d} B',
              file=sys.stderr)
    return ('/* Generated by tools/build-pin-art.py from the artwork in artwork/.\n'
            '   Do not hand-edit: run the script. `--check` proves this file still\n'
            '   matches the artwork it was built from. */\n'
            'window.__PIN_ART__ = {\n' + '\n'.join(rows) + '\n};\n')


if __name__ == '__main__':
    """--check exists because this file's OUTPUT is committed next to it.

    A generated file living in the repository drifts from its generator the moment
    somebody edits the artwork and forgets to re-run this - and the drift is
    invisible, because the stale pin-art.js still loads and still draws pins. They
    are just the OLD pins, and the map looks entirely healthy. So the build is
    re-run in the test suite and compared, which turns a silent drift into a
    failing assertion.
    """
    args = [a for a in sys.argv[1:] if a != '--check']
    out = main(args[0] if args else '.')
    if '--check' in sys.argv:
        here = os.path.dirname(os.path.abspath(__file__))
        target = os.path.join(here, '..', 'pin-art.js')
        have = open(target, encoding='utf-8').read() if os.path.exists(target) else ''
        if have != out:
            print('FAIL pin-art.js does not match the artwork in artwork/ - re-run '
                  'tools/build-pin-art.py', file=sys.stderr)
            sys.exit(1)
        print('  ok   pin-art.js is in step with the artwork it was built from')
    else:
